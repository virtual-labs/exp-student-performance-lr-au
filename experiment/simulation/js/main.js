document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("manualPredictBtn")
    .addEventListener("click", predictManualEntry);
  document
    .getElementById("dataCleanBtn")
    .addEventListener("click", cleanDataSets);
  document.getElementById("splitDataBtn").addEventListener("click", splitData);
  document
    .getElementById("predictBtn")
    .addEventListener("click", predictAndPlot);
...
function updateStatistics() {
  ...
  let meanHours = (
    studyHours.reduce((a, b) => a + b, 0) / totalRecords
  ).toFixed(2);
...
function cleanDataSets() {
  ...
  document.getElementById("splitDataBtn").style.display = "block";
...
function removeOutliers() {
  ...
  let filteredData = studyHours
    .map((sh, i) => ({ study: sh, score: scores[i] }))
  ...
  logActivity(
    "✅ Data Cleaned! Removed Outliers using Interquartile Range (IQR) Method."
  );
...
function splitData() {
  ...
  logActivity(
    `✂️Split data into Training (${trainData.length} points) and Testing (${testData.length} points).`
  );
...
function predictManualEntry() {
  ...
  document.querySelector(".card.upload-container").style.display = "none";
  ...
  const predictions = studyHours.map((hours) => ({
    x: hours,
    y: slope * hours + intercept,
  }));

  // Calculate MSE & RMSE
  const { mse, rmse } = calculateMSEAndRMSE(
    scores,
...
function plotManualEntryGraphHour(studyHours, actualScores, predictions = []) {
  ...
  const filteredActualData = studyHours
    .map((h, i) => {
    ...
    data: {
      datasets: [
        ...
        },
        {
          ...
          data: [
            {
              x: studyHours[studyHours.length - 1],
              y: actualScores[actualScores.length - 1],
            },
          ],
  ...
  if (outliers.length > 0) {
    logActivity(
      ` Outlier Detected: ${outliers
        .map((o) => `Study Hours: ${o.x}, Score: ${o.y}`)
        .join(" | ")}`
    );
  }
}
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      datasets: [
        {
          label: "Training Data",
          data: trainHours.map((hours, index) => ({
            x: hours,
            y: trainScores[index],
          })),
          backgroundColor: "blue",
